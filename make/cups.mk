...
CUPS_DEPENDS=libjpeg, libpng, libtiff, openssl, zlib, psmisc, python26
...
#
CUPS_PATCHES=$(CUPS_SOURCE_DIR)/cups-1.5.4-man-Makefile.patch $(CUPS_SOURCE_DIR)/cups-1.5.4-ppdc-Makefile.patch \
	$(CUPS_SOURCE_DIR)/cups-1.5.4-no_pie-Makedefs.patch
...
#	$(MAKE)   avahi-host-stage
	...
	(cd $(@D); \
		...
		--with-python \
		--enable-dnssd \
		--with-dnssd-includes=/usr/include/avahi \
	  	--with-dnssd-libs=/lib \
		--disable-slp \
		--disable-gnutls \
		--disable-gssapi \
...
ifeq (openldap, $(filter openldap, $(PACKAGES)))
	...
	(cd $(@D); \
		...
		--enable-dnssd \
		--enable-tiff \
		--with-pdftops=/opt/bin/pdftops \
		--with-dnssd-includes=$(CUPS_SOURCE_DIR)/ \
	  	--with-dnssd-libs=$(STAGING_DIR)/opt/lib \
		--disable-slp \
		--disable-gnutls \
		--disable-gssapi \
	)
...
# Copy the init.d startup file
	install -d $(CUPS_IPK_DIR)/opt/etc/init.d
	install -m 755 $(CUPS_SOURCE_DIR)/S88cupsd $(CUPS_IPK_DIR)/opt/etc/init.d/S88cupsd
...
$(CUPS_DOC_IPK): $(CUPS_BUILD_DIR)/.built
	...
	install -d $(CUPS_IPK_DIR)/opt/share/doc/cups
	install -d $(CUPS_IPK_DIR)/opt/man
	cp -rf $(CUPS_BUILD_DIR)/install/opt/man/man1 \
		$(CUPS_IPK_DIR)/opt/man
	...
	cp -rf $(CUPS_BUILD_DIR)/install/opt/share/doc/cups/*.css \
		$(CUPS_IPK_DIR)/opt/share/doc/cups
