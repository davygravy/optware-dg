...
HPLIP_DEPENDS=python26, libstdc++, libusb
...
#  DISABLE for 3.11.10 and higher!
#HPLIP_PATCHES = $(HPLIP_SOURCE_DIR)/hplip-cups-1.5.0-compatibility.patch
#HPLIP_PATCHES = $(HPLIP_SOURCE_DIR)/hplip-state-dir_fix.patch
...
$(HPLIP_BUILD_DIR)/.configured: $(DL_DIR)/$(HPLIP_SOURCE) $(HPLIP_PATCHES) make/hplip.mk
	$(MAKE) cups-stage dbus-stage python-stage  libusb-stage
ifneq (, $(filter net-snmp, $(PACKAGES)))
	$(MAKE) net-snmp-stage
endif
...
$(HPLIP_BUILD_DIR)/.built: $(HPLIP_BUILD_DIR)/.configured
	rm -f $@
	$(MAKE) -C $(@D) PYTHONINCLUDEDIR="$(STAGING_INCLUDE_DIR)/python2.6"
	touch $@
